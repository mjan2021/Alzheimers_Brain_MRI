...
if __name__ == '__main__':
    ...
    dataset = AlzheimersClassficiation(data_dir, '../data/train.csv', transform=None)
    dataset_size = len(dataset)
    test_dataset = AlzheimersClassficiation(data_dir, '../data/test.csv', transform=None)
...
#                                        'test': test_acc_epoch}, step)
            ...
            if test_acc_epoch > best_acc:
                ...
                torch.save(model.state_dict(),
                           f'../models/{model._get_name()}_{args.epochs}_Epochs.pth')

                # Save Scripted Model 
                scripted_model = torch.jit.script(model)
                torch.jit.save(scripted_model,
                               f'../models/scripted_{model._get_name()}_{args.epochs}_Epochs.pt')
        ...
        np.save(f'../output_files/{model._get_name()}_{epochs}_Fold{fold}.npy', cf_matrix)
